import subprocess
import sys
import os
from subprocess import Popen, PIPE
import csv
import json


prefix = sys.argv[1]
k =  int(sys.argv[2])
max_jobs = int(sys.argv[3])
output_file = sys.argv[4]

canonical_gene_list = list() 
with open(prefix + '_genes.csv', 'r') as f:
	reader = csv.reader(f)
	canonical_gene_list = list(reader)[0]

gene_search_list = list()
for i in range(0, len(canonical_gene_list)): 
	gene_search_list.append(str(i))
intersection_len = len(canonical_gene_list) 

gene_idx_best_so_far_list = list()
gene_name_best_so_far_list = list()
accuracies_list = list()
while(len(gene_idx_best_so_far_list) < intersection_len):
	counter = 0
	new_genes_dict = dict()

	for i in range(0, int(len(gene_search_list)/max_jobs)+1):
		processes = []
		for m in range(0, max_jobs):
			j = i*max_jobs + m
			if (j == len(gene_search_list)):
				break
			args = ['./Precise_Feature_Selection_Recursive', prefix, str(k)]
			args += gene_idx_best_so_far_list
			args.append(str(gene_search_list[j]))
			p = subprocess.Popen(args, stdout=PIPE, stderr=PIPE)
			processes.append(p)

		for p in processes:
			stdout, stderr = p.communicate()
			val = float(stdout.decode("utf-8"))
			new_genes_dict[gene_search_list[counter]] = val
			counter += 1

	ListOfSortedGeneTuples = sorted(new_genes_dict.items(), reverse=True, key=lambda v : v[1])
	accuracies_list.append(ListOfSortedGeneTuples[0][1])
	gene_idx_best_so_far_list.append(ListOfSortedGeneTuples[0][0])
	gene_name_best_so_far_list.append(canonical_gene_list[int(ListOfSortedGeneTuples[0][0])])
	gene_search_list.remove(ListOfSortedGeneTuples[0][0])

	#print(str(len(gene_idx_best_so_far_list)) + " Iteration over for k=" + str(k))
	#print(gene_name_best_so_far_list)
	#print(accuracies_list)
	#sys.stdout.flush()
with open(output_file, 'w') as filehandle:  
    json.dump(gene_name_best_so_far_list, filehandle)
